...
const updatePost = ({ initialData }) => {
  ...
  const initialState = {
    title: initialData?.data.post.title,
    subtitle: initialData?.data.post.subtitle,
    description: initialData?.data.post.description,
    category: initialData?.data.post.category,
    type: initialData?.data.post.type,
  };
  const [formValue, setFormValue] = useState(initialState);
  const { title, subtitle, description, category, type } = formValue;
  const [imageURL, setImageURL] = useState(initialData?.data.post.image);
  ...
  return (
    <Layout home gray>
      <Head>
        <title>Update Post | MyBlogs.com</title>
      </Head>
          ...
          {/* <div className="flex flex-col gap-4 items-center justify-center px-[2px] mb-4">
            ...
            <div className="flex">
              <div className="bg-gray-50 p-2 backdrop-blur-lg bg-black/10 ">
                <div className="absolute top-0   left-0 text-center w-full p-2 z-[1] text-base font-semibold text-gray-700">
                </div>
                <FileBase
                  type="file"
                  className="w-full"
...
export async function getServerSideProps(context) {
  const { blogId } = context.params; // Use `context.params` to get dynamic params//gettig id from dynamic route
